...
function auth_token_check(req, res, next){
 var token = req.body.auth_token;
 try {
	 mongo.connect(dbname, function(err, db){
  	 db.collection('users').find( { _id : Number(req.params.userid) }).toArray(function(err, user){
  		 if(user) {
  			 console.log("user in auth_token");
  			 console.log(user);
  			 console.log(`token is ${token} and ${Base64.encode(user[0].name)} `);
  			 if(token==Base64.encode(user[0].name)) {
  				 return next();
  			 }
  			 return res.status(403).send({
  			        success: false,
...
app.put('/user_info/:userid', login_check, auth_token_check,function(req, res){
	...
	mongo.connect(dbname, function(err, db){
		db.collection('users').findOneAndUpdate(
			...
			if (err) { return err }
			if(userupdate) {
				console.log(userupdate.value.name);
